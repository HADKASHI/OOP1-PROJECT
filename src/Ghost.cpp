...
void Ghost::update(sf::Time delta, sf::Vector2f targetPosition)
{
    ...
    else
    {
        ...
        //    scatter(delta);
        if (direction == this->getDirection() * -1.f)
            alternateMove(delta);
        else
        {
...
void Ghost::alternateMove(sf::Time delta)
{
    ...
    {
        if (m_alternateDirection != LEFT && this->getDirection() != LEFT && rand() % 2 == 0)
        {
            m_alternatePosition = sf::Vector2f({ getPosition().x + getSize().x * 0.7f, getPosition().y });
    ...
    {
        if (m_alternateDirection != UP && this->getDirection() != UP && rand() % 2 == 0)
        {
            m_alternatePosition = sf::Vector2f({ getPosition().x, getPosition().y + getSize().y * 0.7f });
